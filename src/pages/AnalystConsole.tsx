...
const dummyThreats = [
  {
    id: "T-892",
    type: "Deepfake Video (Voice Clone)",
    target: "CEO",
    platform: "Telegram (Dark Channel)",
    status: "pending",
    date: "2026-06-12",
    severity: "Critical",
  },
...
export default function AnalystConsole() {
  ...
  const verifyThreat = (id: string) => {
    setThreats(
      threats.map((t) => (t.id === id ? { ...t, status: "verified" } : t)),
    );
  ...
  const generateDMCA = (threat: (typeof dummyThreats)[0]) => {
    ...
    doc.text(
      "I am writing on behalf of my client to officially notify you of an unauthorized and",
      20,
      130,
    );
    doc.text(
      "malicious use of their likeness, identity, or proprietary data on your platform.",
      20,
      137,
    );

    doc.text(
      "Under the Digital Millennium Copyright Act (17 U.S.C. § 512), and applicable privacy",
      20,
      150,
    );
    doc.text(
      "laws, we demand the immediate and permanent removal of the offending material.",
      20,
      157,
    );
    ...
    doc.text(
      "is not authorized by the intellectual property owner, its agent, or the law.",
      20,
      177,
    );

    doc.text(
      "Failure to comply within 24 hours will result in immediate injunctive relief.",
      20,
      190,
    );
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-8 pb-4 border-b border-white/10">
          <div>
            ...
            <p className="text-white/50 text-sm mt-2">
              Zero False Positives Queue - Internal Analyst Portal
            </p>
          </div>
          <div className="px-4 py-2 bg-primary/10 border border-primary/30 rounded-full text-primary text-xs font-bold uppercase tracking-widest flex items-center gap-2">
            ...
            {threats.filter((t) => t.status === "pending").length} Threats
            Pending Review
          </div>
        </div>
        ...
        <div className="grid gap-4">
          {threats.map((threat) => (
            <div
              key={threat.id}
              className="bg-white/[0.02] border border-white/5 p-6 rounded-xl flex items-center justify-between transition-all hover:bg-white/[0.04]"
            >
                ...
                <div>
                  ...
                  <div className="flex items-center gap-4 text-sm text-white/60">
                    <span className="flex items-center gap-1">
                      <AlertTriangle className="h-3 w-3" /> {threat.target}
                    </span>
                    <span>|</span>
