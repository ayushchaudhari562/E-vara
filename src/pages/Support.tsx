...
const SupportPage = () => {
  ...
  const handleSend = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setChat(prev => [...prev, { role: "bot", content: "Inquiry received. Relaying to your dedicated account lead. Current response time: < 15 minutes." }]);
    }, 1000);
  };
      ...
      <div className="flex-1 container mx-auto px-6 py-12 flex flex-col md:flex-row gap-8 max-w-6xl">
        ...
        <div className="md:w-80 space-y-6">
          <div className="p-6 rounded-[24px] border border-white/5 bg-[#11141B]">
             <h3 className="font-bold uppercase tracking-widest text-xs mb-6">Assigned Operative</h3>
             <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                   ...
                   <p className="text-[10px] text-muted-foreground uppercase">Senior Defense Lead</p>
                </div>
             </div>
             <div className="space-y-4 pt-4 border-t border-white/5">
                ...
                <div className="flex items-center gap-2 text-[10px] text-success uppercase">
                   <div className="h-1.5 w-1.5 rounded-full bg-success animate-pulse" /> Operative Online
                </div>
             </div>
          </div>

          <div className="p-6 rounded-[24px] border border-white/5 bg-white/[0.02]">
             <h3 className="font-bold uppercase tracking-widest text-xs mb-6">Resources</h3>
        ...
        <div className="flex-1 flex flex-col rounded-[24px] border border-white/10 bg-[#11141B] overflow-hidden relative shadow-2xl shadow-primary/5">
          ...
          <div className="flex-1 p-6 overflow-y-auto space-y-6 relative z-10 min-h-[400px]">
            {chat.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[80%] rounded-2xl p-4 text-xs font-mono ${
                  msg.role === 'user' 
                  ...
                  {msg.role === 'bot' && <div className="flex items-center gap-2 mb-2 text-[9px] font-bold text-primary uppercase tracking-widest"><Bot className="h-3 w-3" /> Operative_OS</div>}
                  {msg.content}
                </div>
              </div>
            ))}
          ...
          <form onSubmit={handleSend} className="p-6 border-t border-white/5 relative z-10 bg-[#11141B]/80 backdrop-blur-md">
            <div className="flex gap-4">
              ...
              />
              <Button type="submit" className="bg-primary hover:bg-primary/90 text-white rounded-xl h-12 w-12 flex items-center justify-center">
                <Send className="h-5 w-5" />
              </Button>
            </div>
            <p className="mt-4 text-[8px] text-center text-muted-foreground uppercase tracking-[0.2em]">
              Encrypted with end-to-end PGP protocols. No logs stored on public clusters.
            </p>
          </form>
        </div>
      </div>
